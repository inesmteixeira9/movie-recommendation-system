...
def read_data(data_path: str, file_name: str = None) -> pd.DataFrame:
    ...
    if file_name is None:
        log.print_log(LogLevel.INFO, f"Reading all files in {data_path}...")
...
def split_data(X_id, y, train_size, test_size, cv=True):
    """Split data into training, testing, and optionally validation sets.

    Args:
        X_id (array-like): Feature data.
        y (array-like): Labels.
    ...
    if cv == True: 
        ...
        X_train, X_test, y_train, y_test = train_test_split(X_temp, y_temp, test_size=test_val_ratio, random_state=42)
